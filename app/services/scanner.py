...
from services.breakout import prepare_ohlcv_df, calculate_breakout
...
async def fetch_and_analyze_stock(iscd: str, name: str, market_div: str, request_params):
    """
    ...
    try:
        ...
        # Batch 2: 이전 120분
        oldest = batch1[-1]
        _, prev_time = get_prev_minute(oldest['stck_bsop_date'], oldest['stck_cntg_hour'])
...
async def get_breakout_rankings(market: str, breakout_request):
    ...
    tasks = [
        fetch_and_analyze_stock(s['mksc_shrn_iscd'], s['hts_kor_isnm'], "J", breakout_request) 
        for s in stocks
    ]
