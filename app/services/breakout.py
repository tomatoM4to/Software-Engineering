...
def calculate_breakout(df: pd.DataFrame, request) -> dict[str, Any]:
    ...
    if df.empty or len(df) < max(
        {request.anchor_ma, *request.target_mas, request.trend_ma or 0}
    ):
        return {
    ...
    for target_ma in request.target_mas:
        ...
        df[col] = (
            abs(df[f"MA{target_ma}"] - df[f"MA{request.anchor_ma}"])
    ...
    converged = (
        bool(is_recently_converged.iloc[target_idx])
        if pd.notna(is_recently_converged.iloc[target_idx])
        else False
    )
    broken_out = (
        bool(is_breaking_out.iloc[target_idx])
        if pd.notna(is_breaking_out.iloc[target_idx])
        else False
    )
    vol_spiked = (
        bool(is_volume_spiked.iloc[target_idx])
        if pd.notna(is_volume_spiked.iloc[target_idx])
        else False
    )
    vol_strong = (
        bool(is_volume_strong.iloc[target_idx])
        if pd.notna(is_volume_strong.iloc[target_idx])
        else False
    )
    trend_ok = (
        bool(trend_filter.iloc[target_idx])
    ...
    formatted_date = (
        signal_date.strftime("%Y-%m-%d %H:%M:%S")
    ...
    return {
        ...
        "convergence_score": round(float(latest["convergence_score"]), 4)
        if pd.notna(latest["convergence_score"])
        else None,
    }
