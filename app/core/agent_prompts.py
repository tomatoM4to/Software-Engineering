"""
AI 에이전트 의사결정 모듈 — System Prompt 정의

보수적 AI: 리스크 회피 중심 — 다중 조건 만족 시만 BUY.
공격적 AI: 타점 돌파 중심 — 분봉 돌파 + 거래량 하나만 있어도 BUY 고려.
뉴스 없으면 차트 단독 분석으로 자동 폴백.
"""
...
- chart_basis: MA, 거래량비율, 등락률, 돌파 여부 수치를 반드시 인용
...
def _fmt(v: object) -> str:
    """None → N/A, bool → 예/아니오, float → 천단위+소수점2자리, int → 천단위"""
    if v is None:
        return "N/A"
...
def _build_news_section(news_context: object) -> str:
    ...
    if not items:
        return "뉴스 수집 완료이나 결과 없음 → 차트 단독 분석으로 진행하세요."

    lines: list[str] = []
    sentiment = getattr(news_context, "overall_sentiment", None)
    if sentiment is not None:
        lines.append(f"- 전체 감성 점수: {sentiment:.2f}")
    lines.append(f"- 수집 건수: {len(items)}건")
    lines.append("")
    lines.append("### 최신 뉴스")
    ...
    lines.append("")
    lines.append("위 뉴스를 바탕으로 news_basis를 작성하세요.")
    return "\n".join(lines)
