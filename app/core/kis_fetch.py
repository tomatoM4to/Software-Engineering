...
class APIResp:
    ...
    def print_all(self):
        if not self._is_success:
            ...
            logger.error("Status Code: %s | Message: %s", self._rescode, self._err_message)
            return
...
# -------------------------------------------------------------------------
_kis_queue: Optional[asyncio.PriorityQueue] = None
_kis_worker_task: Optional[asyncio.Task] = None
...
async def start_kis_worker():
    ...
    if _kis_queue is None:
        ...
        logger.info("[KIS Async Worker] Started background worker for API rate limiting (20 req/s).")
