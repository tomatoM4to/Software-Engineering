...
class CacheScheduler:
    ...
    async def update_rankings(self, force: bool = False):
        ...
        for market in ["J", "Q"]:
            try:
                ...
                res = await get_volume_rank(market_div=market, bypass_cache=True, priority=10)
                if res.is_ok():
                    stocks = res.get_body().output[:30]
                    await kis_cache.update_ranking(market, stocks)
                else:
                    logger.warning(f"[CacheScheduler] Failed to update {market} ranking: {res.get_error_message()}")
    ...
    async def update_minute_bars(self, force: bool = False):
        ...
        if not force and not self._is_market_open():
            logger.debug("[CacheScheduler] Market is closed. Skipping minute bars update.")
            return

        if self._is_updating:
            logger.warning("[CacheScheduler] Previous update still in progress. Skipping...")
            return
        ...
        try:
            ...
            if not all_stocks:
                logger.info("[CacheScheduler] No stocks in ranking yet. Skipping minute bars update.")
                return
            ...
            for i in range(0, len(all_stocks), batch_size):
                ...
                tasks = [self._fetch_and_cache_minute_bars(code, market) for code, market in batch]
                await asyncio.gather(*tasks)
                # 배치 사이 약간의 휴식 (선택 사항, 큐 워커가 초당 20건 처리하므로 10개씩은 안전함)
            
            logger.info(f"[CacheScheduler] Completed minute bars update for {len(all_stocks)} stocks.")
        finally:
    ...
    async def _fetch_and_cache_minute_bars(self, stock_code: str, market: str):
        ...
        try:
            ...
            batch1 = await fetch_chart_data(stock_code, market_div="J", bypass_cache=True, priority=10)
            if not batch1:
                return
            ...
            batch2 = await fetch_chart_data(stock_code, end_time=prev_time, market_div="J", bypass_cache=True, priority=10)
