    ...
    for i in range(retries):
        try:
            ...
            if response.status_code == expected_status:
                print(f"   {GREEN}✅ {name} is UP ({response.status_code}) - {duration:.2f}s{RESET}")
                return True
            else:
                print(f"   {YELLOW}⚠️ Attempt {i+1}/{retries}: Status {response.status_code}{RESET}")
...
def main():
    ...
    checks = [
        ...
        ("3. 주식 분석 (실시간 시세/차트)", f"{domain}/api/stock/chart/005930", 200, 60),
    ...
    if all_passed:
        ...
        print(f"{RED}{BOLD}⚠️ 일부 시스템에 문제가 발견되었습니다. 로그를 확인하세요.{RESET}\n")
        sys.exit(1)
